...
public class Game2048Layout extends RelativeLayout{
    ...
    public interface OnGame2048Listener {
        ...
        void onGameOver();
        void onVictory(boolean isVictory);
    }
        ...
        for (int j = 0; j < row.size() - 1; j++) {
            ...
            if (item1.getNumber() == item2.getNumber()) {
                ...
                if (mGame2048Listener != null) {
                    ...
                    //如果得到了2048这个数，提示游戏胜利
                    if(val==2048){
                        mGame2048Listener.onVictory(true);
                        return;         //当前停止
                    }
                }
                // 向前移动
        ...
        for (int i = 0; i < mColumn; i++) {
            for (int j = 0; j < mColumn; j++) {
                ...
                if ((index + 1) % mColumn != 0) {
                    ...
                    if (item.getNumber() == itemRight.getNumber()) return false;
                }
                // 下边
                if ((index + mColumn) < mColumn * mColumn) {
                    Game2048Item itemBottom = mGame2048Items[index + mColumn];
                    if (item.getNumber() == itemBottom.getNumber()) return false;
                }
                // 左边
                if (index % mColumn != 0) {
                    Game2048Item itemLeft = mGame2048Items[index - 1];
                    if (itemLeft.getNumber() == item.getNumber()) return false;
                }
                // 上边
                if (index + 1 > mColumn) {
                    Game2048Item itemTop = mGame2048Items[index - mColumn];
                    if (item.getNumber() == itemTop.getNumber()) return false;
                }
            }
        }
        return true;
